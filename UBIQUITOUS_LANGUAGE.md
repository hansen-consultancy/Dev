...
- **"Target".** In code, `BuildSolutionOrProject` now uses `buildTarget` for the "**Solution** file if present, else **Project** file" path, disambiguating it from the MSBuild meaning of *target*. The JSON field in `Step.Data` remains `"target"` for wire stability.

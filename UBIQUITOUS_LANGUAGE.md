...
| **Built-in Command** | A **Command** whose canonical facts (name, **Aliases**, description, **Needs Workspace**, default-ness, **Arg Syntax**) are declared as a `BuiltinCommand` record in the **Command Catalog**: `launch`, `bump`, `bump-commit`, `build`, `frontend`, `synchronize`, `clean`, `help`. Its dispatch arm lives in `Program.cs`, kept in sync with the catalog by an exhaustiveness (drift) guard. | Default command, native command |
...
| **Alias** | A short synonym for a **Built-in Command** (`b`, `c`, `h`/`?`, `f`, `s`/`sync`, `v`, `vc`). | Shortcut, abbreviation |
...
| **Needs Workspace** | Whether a **Built-in Command** requires a detected **Workspace** before it can run: `launch`, `bump`, `bump-commit`, `build`, and `synchronize` do; `frontend`, `clean`, and `help` do not. A **Command** that needs a **Workspace** but finds none produces a failed **Step** with `Code: no_project_found` (exit 2). | Requires project, needs solution |
...
| **Vidyano App** | The one **Project** in a **Workspace** that is a Vidyano application — recognized by the model file the Vidyano service persists beside it (`App_Data/model.json`, or `wwwroot/App_Data/model.json`), never by its package references. The target of the `synchronize` **Command**; when a **Solution** holds several, the **Run**'s first argument names one. | Web project, service project |
| **Skip Reason** | Why a **Project Candidate** was excluded from a **Bump**: `submodule` (the checkout belongs to another repository — a `.git` file whose `gitdir:` names a `modules` segment; a linked worktree's `.git` file is *not* one) or `ignored` (listed in **Dev Config**'s `IgnoreProjects`). | Exclusion cause |
...
| **Vidyano Service CLI** | The Vidyano service's own command-line mode, invoked by the `synchronize` **Command** as `dotnet run --project <Vidyano App> -- --vidyano-update-model=yes --vidyano-synchronize-schema=yes`. Both switches are database-free and idempotent, and the service exits once they have run instead of serving. | Synchronize tool, sync CLI |
| **Synchronize** | The act of running the **Vidyano Service CLI** against the **Vidyano App** so its persisted model and schema files match the code. Distinct from **Build** (compilation) and from a **Bump** (versioning). | Sync, update model |
...
- A **Workspace** contains at most one **Vidyano App** for the `synchronize` **Command** to target; zero fails the **Step** with `no_vidyano_project` (exit 2) and more than one with `ambiguous_vidyano_project` (exit 5) until the **Run** names one.
...
- **"Synchronize" (two switches, one verb).** The `synchronize` **Command** covers both of the **Vidyano Service CLI**'s operations: `--vidyano-update-model=yes` (rewrites the model after a Vidyano *dependency* upgrade) and `--vidyano-synchronize-schema=yes` (reconciles the model with *entity/code* changes, and normalizes `culture.json`/`security.json`). Vidyano's own docs call only the second one "synchronize". Recommendation: **Synchronize** is the CLI's single verb for "make the persisted model match reality"; name the individual switches (**update-model**, **synchronize-schema**) when the distinction matters, and never call either a **Build**.
